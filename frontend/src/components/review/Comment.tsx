...
export default function Comment({
  ...
  return (
    <div className={`px-1 py-1 border-b ${isReplyComment ? "ml-3" : ""}`}>
      <div className="flex justify-between">
        <div className="flex items-center">
          ...
          />
          <Link to={`/user/${comment.author_email}`}>
            <p className="ml-2 font-medium">{comment.author_name}</p>
          </Link>
        </div>
        <div>
      ...
      <div className="">
        ...
        <div className="flex mt-1">
          ...
          <button
            ...
            onClick={() => replyHandler(comment.id, comment.author_name)}
          >
            Reply
          </button>
        </div>
      </div>
      {comment.has_replies && showReplies && !isReplyComment && (
        <p
